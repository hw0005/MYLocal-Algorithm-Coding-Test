package Day260907.다익스트라복습.K번째최단경로구하기;
...
public class K번째최단경로구하기 {
	public static void main(String[] args) throws IOException {
		...
		for (int i=0; i<m; i++) {
			...
			list[start].add(new Edge(node, value));
		}
		...
		while (!pq.isEmpty()) {
			...
			for (Edge next : list[nowNode]) {
				int cost = now.value + next.value;
				
				if (dq[next.node].size() < k) {
					dq[next.node].add(cost);
					pq.offer(new Edge(next.node, cost));
				}
				else if (dq[next.node].peek() > cost) {
					dq[next.node].poll();
					dq[next.node].add(cost);
					pq.offer(new Edge(next.node, cost));
				}
